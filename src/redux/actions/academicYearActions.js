...
export const academicFeesStudentDiscountData = (updateYearOfFee, updateDiscountFeeType, values, setEditingYearOfFee, getapi) => async (dispatch) => {
    try {
        getAccessToken();
...
export const academicYearStudentYearData = (id, student_id) => async (dispatch) => {
    try {
        getAccessToken();
...
export const academicFeesSetAcademicYearData = (id, student_id, setAcademic, setAcademicYear) => async (dispatch) => {
    try {
        getAccessToken();
...
export const academicFeesSetFeeMasterIdData = (setfeemasterid) => async (dispatch) => {
    try {
        getAccessToken();
        const {
            data: { data },
...
export const academicfeesStudentDiscountData2 = (id, academicYear, termsmaster) => async (dispatch) => {
    try {
        getAccessToken();
...
export const academicFeesSchoolDetailsData = (setGotSchoolDetails) => async (dispatch) => {
    try {
        getAccessToken();
        const {
            data: { data },
