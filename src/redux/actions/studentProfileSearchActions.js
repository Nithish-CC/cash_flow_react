...
import { ActionTypes, studentProfileSearchwithid, studentProfileSetAllSection } from "../constants/action-types";
...
export const stuProfileSearchSetAllSection = (setAllSection) => async (dispatch) => {
    try {
        const {
            data: { data },
...
export const studentProfileSearchWithIdData = (id, studentProfileParameters) => async (dispatch) => {
    try {
        const response = await axios.put(`${baseUrl}studentProfile/${id}`, studentProfileParameters).then((res) => {
