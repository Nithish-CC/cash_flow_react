...
const Studentpay = () => {
    ...
    const studentpayautosearchdatas = useSelector((state: any) => state.studentDetailsGet.studentPay_AutosearchData);

    console.log(studentpayautosearchdatas);

    const feemasteriddata = useSelector((state: any) => state.studentDetailsGet.studentPay_feeMaster);

    console.log(feemasteriddata);

    const studentpaytermschangedata = useSelector((state: any) => state.studentDetailsGet.studentPay_paymentapi?.data?.data);
        ...
        if (academicYear && academicYear.length > 0) {
            if (searchBy && searchBy.length > 0) {
                if (searchBy && searchBy.length > 0 && academicYear && academicYear.length > 0) {
                    dispatch(studentPayAutosearchData(searchBy, academicYear, setMainSearch, setadmissionsid, termsChange));
                }
            }
        } else {
    ...
    const handleBalance = () => {
        ...
        if (FeetempArr && FeetempArr.length == 0) {
            ...
            dispatch(studentPayHandleBalanceData(FeetempArr, toast, windowReload, setButtonDisable));
            dispatch(studentPayUpdateStudentBalanceData(FeetempArr, toast, windowReload, setButtonDisable));
        }
    };
    ...
    const handleRefund = () => {
        ...
        if (FeetempArr && FeetempArr.length == 0) {
            ...
            dispatch(studentPayUpdateStudentBalanceData2(FeetempArr, toast, windowReload, setButtonDisable));
        }
    };
    useEffect(() => {
                    ...
                    <div>
                        <div id="content">
                            ...
                            <div className="container">
                                                ...
                                                {!refundSwitch ? (
                                                    <a style={{ color: "rgb(230, 63, 63)" }}>
                                                        <h4 className="mb-0">
                                                            ...
                                                            >
                                                                <Form.Control value={AllDetailsOfStudent && AllDetailsOfStudent.length && AllDetailsOfStudent[0].Allbalance} disabled />
                                                            </div>
                                                        </h4>
                                                    </a>
                                                ) : (
                                                    ...
                                                    <div className="card-body bg-transparent tableFixHead">
                                                        <Table striped bordered hover>
                                                            ...
                                                            <tbody className="pointer">
                                                                {studentpaytermschangedata &&
                                                                    studentpaytermschangedata[0]?.map((value: any, index: any) => {
                                                                        return (
                                                                            <tr key={index}>
                                                                                            ...
                                                                                            onChange={(e: any) => {
                                                                                                Number(e.target.value) > Number(value.balance)
                                                                                                    ...
                                                                                                    : handlePriceChange({
                                                                                                          ...
                                                                                                          amount_paid: Number(e.target.value) + Number(value.amount_paid),
                                                                                            ...
                                                                                            onChange={(e: any) => {
                                                                                                Number(e.target.value) > Number(value.amount_paid)
                                                                                                    ...
                                                                                                    : handleRefundPriceChange({
                                                                                                          index: index,
                                                                                                          amoundTyped: Number(Math.round(e.target.value)),
                                                                                                          amount_paid: Number(value.amount_paid) - Number(e.target.value),
                                                                                                          student_payment_info_id: value.student_payment_info_id,
                                                                                                          grade_id: value.grade_id,
                                                                                                          student_id: value.student_id,
                                                            ...
                                                            <tfoot>
                                                                ...
                                                                studentpaytermschangedata[0].length ? (
                                                                    <tr>
                                                                        <th>Total</th>
                                                                        <th id="totalfeeamt">
                                                                            {studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[4].totalFees}
                                                                        </th>
                                                                        <th>{studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[5].totalcumamt}</th>
                                                                        <th>{studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[2].totalRefund}</th>
                                                                        <th>{studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[6].totaldiscountamount}</th>

                                                                        <th />
                                                                        <th>{studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[1].balance}</th>
                                                                        {!refundSwitch ? (
                                                                            <th style={{ padding: "13px" }} id="totalbalamt">
                                                                                {studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[1].Allbalance}
                                                                            </th>
                                                                        ) : (
                                                                            <th id="totalpaidamt">
                                                                                {" "}
                                                                                {studentpaytermschangedata && studentpaytermschangedata.length && studentpaytermschangedata[2].Alltotalpaid}
                                                                            </th>
                                                                        )}
                                                                    </tr>
                                                                ) : (
                                ...
                                <Listofpayment feemasterid={feemasteriddata} studentdetails={AllDetailsOfStudent} studentadmission={AllDetailsOfStudent}></Listofpayment>
                            </div>
                        </div>
                    </div>
                </div>
