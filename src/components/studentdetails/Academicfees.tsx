...
const Academicfees = (props: any) => {
    ...
    const studentYearData = useSelector((state: any) => state.studentDetailsGet.academicFees_studentYear?.data?.data);
    ...
    const studentDiscountData = useSelector((state: any) => state.studentDetailsGet.academicFees_studentDiscount?.data?.data);
    ...
    const discountType = useSelector((state: any) => state.studentDetailsGet.academicFees_DiscountType);
    console.log(discountType);

    const feeId = useSelector((state: any) => state.studentDetailsGet.feesDetails_SetFeeMasterId);
    ...
    const handlespechar = (values: any, char: any) => {
        ...
        if (result && result.length >= 1) {
            ...
            dispatch(academicFeesStudentDiscountData(updateYearOfFee, updateDiscountFeeType, values, setEditingYearOfFee, getapi));
        }
    };
    useEffect(() => {
    ...
    function studentyear(gradedata: any) {
        var matchedyearid: any = feemasterid && feemasterid.length && feemasterid.filter((data: any) => data.fee_master_id === gradedata.fee_master_id);
    ...
    function setdiscountt(gdata: any) {
        var matcheddiscou: any = discountType && discountType.length && discountType.filter((data: any) => data.dis_feetype_id === gdata.dis_feetype_id);
        ...
        feeId.map((value: any) => {
            discountallrecord &&
                ...
                discountallrecord.map((feeTypeName: any) => {
                    if (feeTypeName.fee_master_id === value.fee_master_id) feeTypeName.fee_type_name = value.fee_type_name;
                });
        });
                    ...
                    <div className="card shadow mb-4">
                        <div className="card-header py-3">
                            ...
                            <Col sm="2" style={{ float: "right", marginRight: "10px" }}>
                                <Form.Select onChange={(e: any) => setAcademicYear(e.target.value)}>
                                    {academic &&
                                        academic.length &&
                                        academic.map((value: any, i: any) => {
                                            return (
                                                <>
                                                    <option value={value.year_id}>{value.academic_year}</option>
                                                </>
                                            );
                                        })}
                                </Form.Select>
                                    ...
                                    <Table striped bordered hover width="100%" style={{ width: "100%" }}>
                                                    ...
                                                    return (
                                                        <tr style={{ textAlign: "center" }}>
                                                            ...
                                                            {index !== editingdiscount.id ? (
                                                                ...
                                                                <td>
                                                                    <Form.Control
                                                                        ...
                                                                        onChange={(e: any) => {
                                                                            Number(values.balance) < Number(e.target.value)
                                                            ...
                                                            {index !== editingdiscount.id ? (
                                                                ...
                                                                <td>
                                                                    <Form.Select value={discountType} onChange={(e) => setUpdateDiscountFeeType(e.target.value)}>
                                                                        {discountType &&
                                                                            discountType.length &&
                                                                            discountType.map((value: any, i: any) => {
                                                                                return (
                                                                                    <>
                                                                                        {value.dis_feetype_name !== "No Discount" && (
                                                                                            <option value={value.dis_feetype_id}>{value.dis_feetype_name}</option>
                                                                                        )}
                                                                                    </>
                                                                                );
                                                                            })}
                                                                ...
                                                                <>
                                                                    <td>
                                                                        <Button
                                                                            variant="warning"
                                                                            onClick={() => {
                                                                                dispatch(academicFeesSetAcademicYearData(id, student_id, setAcademic, setAcademicYear));
                                                                        ...
                                                                        <>
                                                                            <td>
                                                                                <Button
                                                                                    variant="primary"
                                                                                    onClick={() => {
                                                                                        setUpdateYearOfFee(Number(values.discount_amount));
                                                                                        setEditingYearOfFee({
                                                                                            id: index,
                                                                                        });
                                                                                    }}
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <th style={{ textAlign: "center" }}>{total.Total_initial_fees}</th>
                                                <th style={{ textAlign: "center" }}>{total.Total_balance}</th>
                                                <th style={{ textAlign: "center" }}>{total.Total_discount}</th>
